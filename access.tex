...
In this study, we employ a Resnet50 model to classify the images into two categories (Authentic and Tampered), we use a transfer learning approach based on the ResNet50 architecture, pre-trained on ImageNet to solve our classification problem. This method capitalizes on the robust feature extraction capabilities of ResNet50, significantly reducing training time and computational resources while improving model accuracy. The model was customized to adapt the pre-trained base for the task by introducing additional trainable (Custom) layers specifically designed for binary classification.Figure ~\ref{base-model}: shows the architecture of the base model we have used featuring Resnet50 in a five-stage design to represent the overall

The base of our architecture is the ResNet50 model, known for its depth and efficient handling of the vanishing gradient problem through residual connections. The model was loaded without the top layers, removing the full-connected top layers helped us retained only the convolutional layers which are responsible for extracting hierarchical features from the input images. The input shape was fixed to match the requirement of the model while ensuring compatibility with pre-trained weights. All layers in the base model were frozen during training to preserve the learned weights, ensuring that the feature extraction capabilities remain intact and are not altered by the new task-specific data. We added five different types of custom layers to the base pre-trained model to our binary classification problem for it to adapt. The first one is a global average pooling layer to reduce the high-dimensional feature maps output by ResNet50 into a single vector for each feature map, thereby reducing the number of trainable parameters and improving generalization. Unlike the traditional Flatten layer, this approach preserves spatial information and improves model performance with ResNet-based architectures. Further, we added batch normalization to normalize the activations from the pooling layer, reducing internal covariate shift and accelerating convergence. 

Once we are done with that, we added a series of two dense layers. The first layer consists of 512 neurons providing substantial capacity to capture complex patterns. The second one contains 256 neurons, further refining feature extraction. Both the layers are Rectified Linear Unit) ReLU activated, which allows gradients to flow effectively in deep networks and outputs zero for negative inputs, creating sparsity in the activations, which can lead to more efficient computations and reduced overfitting. While being computationally less expensive than sigmoid or tanh function. The custom layers then involved a dropout regularization technique which was employed after each dense layer to with a rate of 0.75 after the first dense layer to impose strong regularization and rate of 0.5 was applied after the second dense layer for moderate regularization. This helped in preventing overfitting of the model during training. After dropout regularization, a single neuron with sigmoid activation was used in the output layer to predict the probability of the positive class, suitable for binary classification tasks.

This architecture provides an optimal trade-off between accuracy, training efficiency, and robustness, making it highly effective for real-world applications. We trained the model for over 100 epochs with step size of around 145 while capturing essential evaluation metrics like accuracy, f1 score and validation accuracy. This ensured we will be able to reduce the gap between the validation and training accuracy of the model making it more robust and highly effective for real-world applications.
...
See \cite{chandio2022precise,chandio2022precise}.
...
See \cite{chandio2022precise}--\cite{chandio2022precise}.

\item \emph{Basic format for reports:}\\
J. K. Author, ``Title of report,'' Abbrev. Name of Co., City of Co., Abbrev. State, Country, Rep. \emph{xxx}, year.\\
See \cite{chandio2022precise}.
...
\end{enumerate}
See \cite{b27}--\cite{chandio2022precise}.
